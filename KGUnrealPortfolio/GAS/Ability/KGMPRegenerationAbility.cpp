...
#include "KGMPRegenerationAbility.h"
...
void UKGMPRegenerationAbility::TickRegeneration()
{
	const UKGCharacterAttributeSet* ownerAttr = mSourceASC->GetSet<UKGCharacterAttributeSet>();
	const float RegenrationValue = ownerAttr->GetMPRecovery();
	FGameplayEffectSpecHandle CostSpecHandle = MakeOutgoingGameplayEffectSpec(UGE_MP::StaticClass(), GetAbilityLevel());
	CostSpecHandle.Data->SetSetByCallerMagnitude(FGameplayTag::RequestGameplayTag(TEXT("Custom.Effect.Common.MP")), RegenrationValue);
